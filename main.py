...
for row in sheet.rows:
    row_id = row.id
    tkt = get_cell_by_column_name(row, 'Equipment Ticket/PWO').display_value
...
sh run
    """
        if [i for i in (
                hostname, password, carrier, cktId,
                account, wanIp, wanSubnet, lanGateway,
                lanSubnet, wanGateway) if i is None]:
            print(f"Missing data in Smartsheet for {tkt}.")
        else:
            attach_file()
...
!
    ...
    """
        if [i for i in (
                hostname, cktId, account, lanGateway,
                lanSubnet, wanIp, wanSubnet, wanGateway,
                city, state) if i is None]:
            print(f"Missing data in Smartsheet for {tkt}.")
        else:
            attach_file()
...
!
    ...
    """
        if [i for i in (
                hostname, lanGateway, lanIp, lanSubnet,
                cktId, account, wanIp, wanSubnet,
                wanGateway, city, state) if i is None]:
            print(f"Missing data in Smartsheet for {tkt}.")
        else:
            attach_file()
    else:
        print(f"Config template not found for {tkt}. Please proceed manually.")
